...
void display()
{
	...
	if (trans_state == 0)
	{
		if (!spacestate)
		{
			...
			{
				glPushMatrix();
				glTranslatef(cow2wld.m[12],cow2wld.m[13],cow2wld.m[14]);
				cx = cameras[cameraIndex][8]*(cameras[cameraIndex][1]-cameras[cameraIndex][4])-cameras[cameraIndex][7]*(cameras[cameraIndex][2]-cameras[cameraIndex][5]);
				cy = cameras[cameraIndex][6]*(cameras[cameraIndex][2]-cameras[cameraIndex][5])-cameras[cameraIndex][8]*(cameras[cameraIndex][0]-cameras[cameraIndex][3]);
				cz = cameras[cameraIndex][7]*(cameras[cameraIndex][0]-cameras[cameraIndex][3])-cameras[cameraIndex][6]*(cameras[cameraIndex][1]-cameras[cameraIndex][4]);
	...
	else
	{
		if (!spacestate)
		{
			...
			{	
				glPushMatrix();
				glLoadIdentity();
				glTranslatef(-cameras[cameraIndex][0]+cow2wld.m[12],-cameras[cameraIndex][1]+cow2wld.m[13],-cameras[cameraIndex][2]+cow2wld.m[14]);
				glMultMatrixd(cam2wld[cameraIndex].matrix());
