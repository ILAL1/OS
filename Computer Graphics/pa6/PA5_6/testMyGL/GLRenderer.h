...
public:
  ...
  int height;
  int levels;
  RGBAColor** data; // pointer to an array of pointers for each level

  Texture() : id(0), data(0), levels(0), width(0), height(0) {}

  // Retrieves the pixel at (x,y) from the indicate mipmap level
  RGBAColor GetPixel( int x, int y, int level = 0 )
  {
  ...
  void SetPixel( int x, int y, int level, const RGBAColor& color )
  {
    assert( x >= 0 && x < (width >> level) && y >= 0 && y < (height >> level) );
