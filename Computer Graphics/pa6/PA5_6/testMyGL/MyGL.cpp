...
{ 
	...
	if( light.enabled )
	{
		...
		{
			//vertex와 light의 position을 가져온다.
			float mposx = verts[i].position[0];
			float mposy = verts[i].position[1];
			float mposz = verts[i].position[2];
			float lposx = light.position[0];
			float lposy = light.position[1];
			float lposz = light.position[2];
			//vector L을 책에 있는 방법으로 계산한다.
			vec3f L;
			float Lvalue = sqrt((lposx-mposx)*(lposx-mposx)+(lposy-mposy)*(lposy-mposy)+(lposz-mposz)*(lposz-mposz));
			L[0] = (lposx-mposx)/Lvalue;
			L[1] = (lposy-mposy)/Lvalue;
			L[2] = (lposz-mposz)/Lvalue;
			//vertex의 normalㄱ을 가져온다.
			vec3f N;
			N[0] = verts[i].normal[0];
			N[1] = verts[i].normal[1];
			N[2] = verts[i].normal[2];
			//vextor R을 책에 있는 방법으로 계산한다. 
			vec3f R;
			R[0] = 2*(N[0]*L[0]+N[1]*L[1]+N[2]*L[2])*N[0]-L[0];
			R[1] = 2*(N[0]*L[0]+N[1]*L[1]+N[2]*L[2])*N[1]-L[1];
			R[2] = 2*(N[0]*L[0]+N[1]*L[1]+N[2]*L[2])*N[2]-L[2];
			float Rvalue = sqrt(R[0]*R[0]+R[1]*R[1]+R[2]*R[2]);
			R[0] = R[0]/Rvalue;
			R[1] = R[1]/Rvalue;
			R[2] = R[2]/Rvalue;

			//vector V를 계산하려면 veiwing position과 vertex의 position이 필요한다. veiwing position이 (0,0,0)이므로 vertex position으로만 계산한다.
			vec3f V;
			V[0] = verts[i].position[0];
			V[1] = verts[i].position[1];
			V[2] = verts[i].position[2];
			float Vvalue = sqrt(V[0]*V[0]+V[1]*V[1]+V[2]*V[2]);
			V[0] = -V[0]/Vvalue;
			V[1] = -V[1]/Vvalue;
			V[2] = -V[2]/Vvalue;
			//ppt의 phong model식으로 계산한다. 
			using std::max;
			using std::pow;
			float zero = 0.0;
			vec4f value;
			value[0] = material.ambient[0]*light.ambient[0]+light.diffuse[0]*material.diffuse[0]*(max(N[0]*L[0]+N[1]*L[1]+N[2]*L[2],zero)) + material.specular[0]*light.specular[0]*pow((max(V[0]*R[0]+V[1]*R[1]+V[2]*R[2],zero)),material.shininess);
			value[1] = material.ambient[1]*light.ambient[1]+light.diffuse[1]*material.diffuse[1]*(max(N[0]*L[0]+N[1]*L[1]+N[2]*L[2],zero)) + material.specular[1]*light.specular[1]*pow((max(V[0]*R[0]+V[1]*R[1]+V[2]*R[2],zero)),material.shininess);
			value[2] = material.ambient[2]*light.ambient[2]+light.diffuse[2]*material.diffuse[2]*(max(N[0]*L[0]+N[1]*L[1]+N[2]*L[2],zero)) + material.specular[2]*light.specular[2]*pow((max(V[0]*R[0]+V[1]*R[1]+V[2]*R[2],zero)),material.shininess);
			value[3] = material.ambient[3]*light.ambient[3]+light.diffuse[3]*material.diffuse[3]*(max(N[0]*L[0]+N[1]*L[1]+N[2]*L[2],zero)) + material.specular[3]*light.specular[3]*pow((max(V[0]*R[0]+V[1]*R[1]+V[2]*R[2],zero)),material.shininess);	
			verts[i].color[0] = value[0];
			verts[i].color[1] = value[1];
			verts[i].color[2] = value[2];
			verts[i].color[3] = value[3];
		}
	}
	return true;
}
					...
					if ((edge3[0]*x+edge3[1]*y+edge3[2]>0) || (edge3[0]*x+edge3[1]*y+edge3[2]==0 && t3))
					{
						...
						if( textureEnabled  )
						{
							if( texture.id != 0 )
							{
								...
								//comment: texture상의 위치를 구한후 linear linterpolation을 이용하여 texture에 있는 색을 구한다. 
								double a = texcoorx * (texture.width - 1);
								double b = texcoory * (texture.height - 1);
								//nearest 방법으로 하려면 바로 아랫줄의 //를 지우고 그다음 줄부터 끝까지 지우면 된다. 
								//color = texture.GetPixel( a, b, 0 );
								int x0 = a;
								int x1 = a+1;
								int y0 = b;
								int y1 = b+1;
								double alpha = (a-x0)/(x1-x0);
								double beta = (b-y0)/(y1-y0);
								RGBAColor color0 = texture.GetPixel( x0, y0, 0 );
								RGBAColor color1 = texture.GetPixel( x1, y0, 0 );
								RGBAColor color2 = texture.GetPixel( x0, y1, 0 );;
								RGBAColor color3 = texture.GetPixel( x1, y1, 0 );;
								color[0] = ((1-alpha)*color0[0]+alpha*color1[0])*(1-beta)+((1-alpha)*color2[0]+alpha*color3[0])*beta;
								color[1] = ((1-alpha)*color0[1]+alpha*color1[1])*(1-beta)+((1-alpha)*color2[1]+alpha*color3[1])*beta;
								color[2] = ((1-alpha)*color0[2]+alpha*color1[2])*(1-beta)+((1-alpha)*color2[2]+alpha*color3[2])*beta;
								color[3] = ((1-alpha)*color0[3]+alpha*color1[3])*(1-beta)+((1-alpha)*color2[3]+alpha*color3[3])*beta;
